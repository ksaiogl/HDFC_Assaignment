...
exports.getPlacesDetails = (req) => {
    return new Promise((resolve, reject) => {
        global.logger.logger_client.info(`${TAG} Inside getPlacesDetails function`)
        if (typeof req.query.placeId == 'string' && req.query.placeId.length > 0) {
            return getPlacesDetailsFromGoogleApi(req)
                ...
                .catch(err => {
                    global.logger.logger_client.info(`${TAG} Error in getPlacesDetails,err:${err.stack}`)
        ...
        if (typeof req.query.quertText == 'string' && req.query.quertText.length > 0) {
            ...
            request({
                ...
                if (!error && response.statusCode === 200) {
                    db.collection('client').updateOne({ 'clientId': req.session.user.userId },
                        {
                            $push: {
                                "searchedPlaces": {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.collection('client').aggregate(invoiceQuery).toArray()
            .then(res => {
                ...
                var result = res.length > 0 ? res[0].searchedPlaces : [];
                return resolve(global.outputResult(result));
            })
            .catch(err => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.collection('client').aggregate(invoiceQuery).toArray()
            .then(res => {
                global.logger.logger_client.info(`Password Reset Succesful`);
                console.log(res);
                var result = res.length > 0 ? res[0].viewedPlaces : [];
                return resolve(global.outputResult(result));
            })
            .catch(err => {
                global.logger.logger_client.error(`Error updating Client Password in DB,err: ${err.stack}`);
